# Language: python
"""
Chroma Flow Studio Flask application.

Provides HTTP endpoints to manage ChromaDB collections, generate embeddings,
import/export data, and visualize collections using ChromaViz.

Contains configuration loaders, embedding initialization logic, and many
Flask routes for collection/document operations.
"""
...
class DefaultConfig:
    """Dataclass for default configuration settings."""

    flask_server_endpoint: str = "http://127.0.0.1:5000"
    proxy_endpoint: str = ""
    chromadb_path: str = (
        "C:\\Program Files\\ChromaDB\\Data\\TestDB"
        if platform.system() == "Windows"
        else "./ChromaDB/Data/TestDB"
    )
    embedding_model_selected_preset: str = "all-MiniLM-L6-v2"
    embedding_model: str = "all-MiniLM-L6-v2"
    embedding_context_window: int = 512
    embedding_model_dimension: int = 768
    embedding_model_path: str = (
        "C:\\Program Files\\ChromaDB\\Data\\models\\all-MiniLM-L6-v2"
        if platform.system() == "Windows"
        else "./models/all-MiniLM-L6-v2"
    )
    language: str = "en-GB"
    cpu_ram_interval_enabled: bool = False
    cpu_ram_interval: int = 2000
    auto_gen_collection_name: str = "testCollection"
    bug_fix_100_documents_enabled: bool = True
    bug_fix_100_documents_batch_size: int = 50000
...
default_config = {
    ...
    "language": "en-GB",
    "cpu_ram_interval_enabled": False,
    "cpu_ram_interval": 2000,
    "auto_gen_collection_name": "testCollection",
    "bug_fix_100_documents_enabled": True,
    "bug_fix_100_documents_batch_size": 50000,
}
...
def load_settings_from_json(file_path):
    ...
    global ADDRESS_FLASK, CHROMA_DATA_PATH, EMBEDMODEL
    global EMBEDMODEL_LOCAL_PATH, EMBEDMODEL_CONTEXTWINDOW, PROXY_URL
    ...
    EMBEDMODEL_CONTEXTWINDOW = settings.get(
        "embedding_context_window", EMBEDMODEL_CONTEXTWINDOW
    )
...
def handle_embedding_init_error(e):
    """
    Handles errors during embedding model initialization.
    ...
    print(
        "Also check your internet connection, any proxy settings, and your connectivity to huggingface."
    )
    print(
        "If all of that is working, then this model is not compatible with ChromaFlow Studio / Chroma, either outright, or without further code changes.."
    )
    # Short summary and prompt user for next steps
    print("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
    print("Unable to load the embedding model.")
    print("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
    user_input = (
        input(
            "\n\nWould you like to continue without transformers model (Embeddings won't work though)? (y/n): "
        )
        .strip()
        .lower()
    )
...
# Function to initialize chroma embedding; Defaulting to using CUDA device fallback to CPU if not available.
def initialize_embedding(device=None):
    """
    ...
    if os.path.exists(EMBEDMODEL_LOCAL_PATH):
        pyEmbedFunction = embedding_functions.SentenceTransformerEmbeddingFunction(
            EMBEDMODEL_LOCAL_PATH, device=device
        )
        print("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
        ...
        print(
            f"Not Found Locally - Downloading model {EMBEDMODEL} from Hugging Face..."
        )
        print(
            "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
        )
        model = SentenceTransformer(EMBEDMODEL, device=device)
...
try:
    ...
    pyEmbedFunction = initialize_embedding()

except Exception as e:

    try:
        if "CUDA error: out of memory" in str(e):
            pyEmbedFunction = initialize_embedding("cpu")
        else:
            handle_embedding_init_error(e)
...
def reinitialize_app():
    ...
    try:
        ...
        initialize_embedding(None)

    except Exception as e:

        error_message = f"Error reinitializing the embedding model: {e}"

        try:
            if "CUDA error: out of memory" in str(e):
                initialize_embedding("cpu")
            else:
                print(error_message)
...
def restart():
    ...
    time.sleep(2)
    return (
        jsonify(message="Server components are being reinitialized. Please wait..."),
        200,
    )


@app.route("/operating-system", methods=["GET"])
def operating_system():
    """Fetches the operating system info"""
    try:
        return jsonify({"operating_system": str(platform.system())})
    except Exception as e:
...
def read_settings():
    """Opens and reads the appsettings.json file and returns its contents."""
    try:
...
def save_settings():
    """Saves updated settings to the appsettings.json file."""
    try:
        ...
        return (
            jsonify({"status": "success", "message": "Settings saved successfully!"}),
            200,
        )
    except Exception as e:
...
def write_default_config_route():
    """Writes a default appsettings.json file to the root folder."""
...
def system_stats():
    """Fetches the system CPU and RAM usage"""
...
def python_stats():
    """Fetches the current Python process CPU and RAM usage"""
...
def chromadb_path_sqllite():
    """Fetches the ChromaDB SQLite data path"""
...
def get_chromadb_app_version():
    ...
    try:
        ...
        command = "findstr" if platform.system() == "Windows" else "grep"
...
def get_embedding_model_info():
    """Fetches the embedding model info"""
...
def flask_heartbeat():
    """Hearbeat endpoint to check if Flask is running"""
    if persistentChromaClient is None:
        # If the ChromaDB client is not initialized, return an error message
        return jsonify({"error": "ChromaDB server is unavailable"}), 500
...
def get_collections():
    """Fetches all collections from ChromaDB"""
    print("=======================================")
    ...
    try:
        ...
        print("   FROM: /get-collections   Checking ChromaDB Version...")
        chromadb_version = get_chromadb_app_version()
        if chromadb_version is None:
            print(
                "   FROM: /get-collections   ERROR: Could not determine ChromaDB version"
            )
            return jsonify({"error": "Could not determine ChromaDB version"}), 500

        print(
            f"   FROM: /get-collections   Confirmed! - ChromaDB version: {chromadb_version}"
        )
        ...
        if version_tuple >= threshold_version:
            print("   FROM: /get-collections   Using Chroma v0.6.0 or higher")
            print(
                "   FROM: /get-collections   Fetching all collections using new method..."
            )
            collections = persistentChromaClient.list_collections()
            collections_list = [{"name": collection} for collection in collections]
        else:
            print("   FROM: /get-collections   Using Chroma v0.5.20 or lower")
            print(
                "   FROM: /get-collections   Fetching all collections using legacy method..."
            )
            collections = persistentChromaClient.list_collections()
            print("   FROM: /get-collections   Collections Fetched...")

            # Convert each collection object into a serializable dictionary
            print(
                "   FROM: /get-collections   Converting Collection to Dictionary Object..."
            )
            collections_list = []
            for collection in collections:
                collections_list.append({"name": collection.name})

        print("   FROM: /get-collections   Returned Fetched Collections as JSON")
...
def count_collections():
    """Fetches the count of collections from ChromaDB"""
    print("=========================================")
    print("Recevied request for: /count-collections")
    print("=========================================")
    ...
    try:
        ...
        print("   FROM: /count-collections   Fetching all Collections...")
        all_collections = persistentChromaClient.list_collections()
        print("   FROM: /count-collections   Collections Fetched")
        ...
        print(
            f"   FROM: /count-collections   Returning Collection Count: {collection_count}"
        )
        return jsonify(collection_count)
...
def get_collection_overview():
    """Fetches an overview of all collections from ChromaDB"""
    print("================================================")
    ...
    if persistentChromaClient is None:
        print(
            "   FROM: /get-collection-overview   ERROR: ChromaDB server is unavailable"
        )
        return jsonify({"error": "ChromaDB server is unavailable"}), 500
    ...
    try:
        ...
        # Check ChromaDB version
        print("   FROM: /get-collection-overview   Checking ChromaDB Version...")
        chromadb_version = get_chromadb_app_version()
        if chromadb_version is None:
            print(
                "   FROM: /get-collection-overview   ERROR: Could not determine ChromaDB version"
            )
            return jsonify({"error": "Could not determine ChromaDB version"}), 500

        print(
            f"   FROM: /get-collection-overview   Confirmed! - ChromaDB version: {chromadb_version}"
        )
        ...
        print("   FROM: /get-collection-overview   Fetching all Collections...")
        all_collections = persistentChromaClient.list_collections()
        print("   FROM: /get-collection-overview   Collections Fetched")
        ...
        if version_tuple >= threshold_version:
            ...
            for collection_name in all_collections:
                try:
                    ...
                    print(
                        f"   FROM: /get-collection-overview   ERROR accessing collection {collection_name}: {str(e)}"
                    )
                    collection_overview.append(
                        {
            ...
            for collection in all_collections:
                try:
                    ...
                    print(
                        f"   FROM: /get-collection-overview   ERROR accessing collection {collection.name}: {str(e)}"
                    )
                    collection_overview.append(
                        {
        ...
        print(
            "   FROM: /get-collection-overview   SUCCESS - Returning Collection Overview as JSON"
        )
        return jsonify(
            {
                "status": "success",
        ...
        print(
            f"   FROM: /get-collection-overview   Error getting Collection Overview: {str(e)}"
        )
        return (
            jsonify(
                {
...
def get_collection_details():
    """Fetches detailed information about a specific collection from ChromaDB"""
    print("===================================================")
    print("Received request for: /api/get-collection-details")
    print("===================================================")

    if persistentChromaClient is None:
        print(
            "   FROM: /api/get-collection-details   ERROR: ChromaDB server is unavailable"
        )
        return jsonify({"error": "ChromaDB server is unavailable"}), 500
    ...
    if not passed_collection_name:
        print("   FROM: /api/get-collection-details   Collection name is required")
        return jsonify({"error": "Collection name is required"}), 400

    print(
        f"   FROM: /api/get-collection-details   Passed Collection: {passed_collection_name}"
    )
    ...
    try:
        ...
        print(
            f"   FROM: /api/get-collection-details   SUCCESS - Returning {passed_collection_name} Details as JSON"
        )
        return jsonify(collection_details)

    except Exception as e:
        print(
            f"   FROM: /api/get-collection-details   Error getting {passed_collection_name} Details: {str(e)}"
        )
        return jsonify({"error": str(e)}), 500
...
def create_new_collection():
    ...
    try:
        ...
        return (
            jsonify(
                {"message": f"Collection '{passed_collection_name}' already exists."}
            ),
            200,
        )
        ...
        print(f"{str(e)}")
        print("Collection not found, proceeding to create it...")
    ...
    try:
        ...
        return (
            jsonify(
                {
                    "message": f"Collection '{passed_collection_name}' created successfully."
                }
            ),
            200,
        )
        ...
        error_message = str(e)
        print(
            f"Error occurred while creating collection '{passed_collection_name}': {error_message}"
        )
...
def clone_new_collection():
    """Clones an existing collection in ChromaDB to a new collection"""
    if persistentChromaClient is None:  # Assuming client is defined elsewhere
        return jsonify({"error": "ChromaDB server is unavailable"}), 500
    ...
    if not passed_collection_name or not new_collection_name:
        return (
            jsonify({"error": "Collection name and new collection name are required"}),
            400,
        )
    ...
    try:
        ...
        return (
            jsonify(
                {
                    "message": f"Collection '{passed_collection_name}' Not Found, aborting..."
                }
            ),
            404,
        )
    ...
    try:
        ...
        return (
            jsonify(
                {
                    "message": f"New Collection Name '{new_collection_name}' already exists, aborting."
                }
            ),
            567,
        )
        ...
        # Handle unexpected errors
        print(
            f"New Collection Name '{new_collection_name}' is available...proceeding..."
        )
    ...
    try:
        ...
        print(
            f"Created new collection '{new_collection_name}' from '{passed_collection_name}' successfully."
        )
    except Exception as e:
    ...
    def chunk_records(doc_ids, documents, metadata, batch_size):
        ...
        for i in range(0, len(doc_ids), batch_size):
            yield doc_ids[i : i + batch_size], documents[i : i + batch_size], metadata[
                i : i + batch_size
            ]

    batches = chunk_records(
        old_doc_ids, old_documents, old_document_metadata, batch_limit
    )

    # Now we need to copy the data in batches
    try:
        print("Copying Data into New Cloned Collection...")
        ...
        for batch_ids, batch_documents, batch_metadata in batches:
            try:
                ...
                pycollection_new.upsert(
                    documents=batch_documents, metadatas=batch_metadata, ids=batch_ids
                )
                print("Upsert for batch successful!")

            except Exception as e:
                print(f"Error during upsert: {str(e)}")
                print("Deleting Cloned Collection to Rollback Changes...")
                ...
                if "exceeds maximum batch size" in str(e):
                    return (
                        jsonify({"error": f"Batch exceeds maximum size: {str(e)}"}),
                        566,
                    )

                return jsonify({"error": f"Failed to upsert documents: {str(e)}"}), 500

        print("Clone Complete - Data migrated successfully.")
        ...
        return (
            jsonify(
                {"error": f"Error migrating documents to Cloned Collection: {str(e)}"}
            ),
            500,
        )
...
def delete_collection():
    ...
    try:
        ...
        return (
            jsonify(
                {
                    "message": f"Collection '{passed_collection_name}' deleted successfully."
                }
            ),
            200,
        )
        ...
        print(f"Error: {str(e)}")
        return (
            jsonify({"error": "An error occurred while deleting the collection"}),
            500,
        )
...
def delete_all_collections():
    ...
    if persistentChromaClient is None:
        print(
            "   FROM: /api/delete-all-collections-v2   ERROR: ChromaDB server is unavailable"
        )
        return jsonify({"error": "ChromaDB server is unavailable"}), 500

    try:
        # Fetch ChromaDB version
        print("   FROM: /api/delete-all-collections-v2   Checking ChromaDB Version...")
        chromadb_version = get_chromadb_app_version()
        if chromadb_version is None:
            print(
                "   FROM: /api/delete-all-collections-v2   ERROR: Could not determine ChromaDB version"
            )
            return jsonify({"error": "Could not determine ChromaDB version"}), 500

        print(
            f"   FROM: /api/delete-all-collections-v2   Confirmed! - ChromaDB version: {chromadb_version}"
        )
        ...
        if not collections:
            print(
                "   FROM: /api/delete-all-collections-v2   No collections found to delete"
            )
        ...
        print(f"Error: {str(e)}")
        return (
            jsonify({"error": "An error occurred while deleting all collections"}),
            500,
        )
...
def generate_embedding():
    ...
    try:
        ...
        return (
            jsonify(
                {"error": f"An error occurred while generating the embedding: {str(e)}"}
            ),
            500,
        )
...
def add_document():
    """Adds a document to a specified collection in ChromaDB"""
    print("=====================================================")
    print("Received request for: /api/add-document")
    print("=====================================================")
    ...
    if not collection_name or not doc_id or not doc_content:
        return (
            jsonify(
                {"error": "Missing required fields (collection_name, id, document)"}
            ),
            400,
        )
    ...
    try:
        ...
        return (
            jsonify({"error": f"Collection {collection_name} not found: {str(e)}"}),
            500,
        )
    ...
    try:
        ...
        pycollection.add(documents=[doc_content], metadatas=[metadata], ids=[doc_id])
...
def add_many_test_documents():
    """Adds multiple test documents to a specified collection in ChromaDB"""
    ...
    try:
        ...
        return (
            jsonify(
                {"error": f"Collection {passed_collection_name} not found: {str(e)}"}
            ),
            500,
        )
    ...
    try:
        print("Trying insert...")
        pycollection.upsert(
            documents=document_texts, metadatas=document_metadatas, ids=document_ids
        )
        return (
            jsonify({"message": "Documents added successfully to the collection."}),
            200,
        )
...
def import_data_file():
    """Imports data into a specified collection in ChromaDB"""
    print("=====================================")
    print("Received Request: /import-data-file")
    print("=====================================")

    if persistentChromaClient is None:
        print("   FROM: /import-data-file   ChromaDB server is unavailable")
        return jsonify({"error": "ChromaDB server is unavailable"}), 500
    ...
    try:
        ...
        return (
            jsonify({"error": "Collection not found. Create the collection first."}),
            500,
        )
    ...
    for batch in batches:
        ...
        if all(not metadata for metadata in document_metadatas):
            try:
                ...
                pycollection.upsert(documents=document_texts, ids=document_ids)
                print("Upsert for batch successful!")
            except Exception as e:
                print(f"Error during upsert: {str(e)}")
                ...
                return (
                    jsonify(
                        {
                            "error": f"Failed to upsert documents without metadata: {str(e)}"
                        }
                    ),
                    500,
                )
...
def gather_export_data():
    """Gathers export data from a specified collection in ChromaDB"""
    print("######################################################")
    print("REQUEST RECEVIED: /gather-export-data")
    print("######################################################")
    ...
    print("Extracting data from JSON Payload...")
    ...
    print("Collection contains documents to export. Proceeding...\n")

    # Gather the fields to include based on the user's selection
    include_fields = []

    print("Building Object for - Content Options")
    ...
    if not include_fields:
        ...
        include_fields.append("documents")
        print(
            "No Content Options were included - falling back to including documents as default"
        )
    ...
    print("Building Object for - Record Options")
    ...
    if record_options == "all_documents":
        ...
        list_of_ids = record_options_values.split(
            ","
        )  # List of IDs in the payload, separated by commas
        ...
        to_record = int(
            record_options_values.get("to", len(results["ids"]))
        )  # Convert 'to' to int
    ...
    print("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
    print("Sending Response Data back to Web App")
    print("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n\n")
...
def export_data_to_json():
    """Exports data from a specified collection in ChromaDB to a JSON file"""
    print("######################################################")
    print("REQUEST RECEIVED: /export-data-to-json")
    print("######################################################")
    ...
    if record_options == "all_documents":
        ...
        try:
            from_record = int(
                record_options_values.get("from", 1)
            )  # Convert 'from' to int
            to_record = int(
                record_options_values.get("to", len(results["ids"]))
            )  # Convert 'to' to int
            ...
            print(
                f"Error: Invalid value for 'from' or 'to'. They must be integers. Error: {e}"
            )
            return (
                jsonify(
                    {
                        "error": 'Invalid value for "from" or "to". They must be integers.'
                    }
                ),
                400,
            )
    ...
    if not os.path.exists(output_directory):
        try:
            ...
            return (
                jsonify({"error": f"Failed to create directory: {output_directory}"}),
                500,
            )
...
def count_collection_documents():
    ...
    try:
        ...
        return (
            jsonify(
                {
                    "error": f"Error accessing collection {passed_collection_name}: {str(e)}"
                }
            ),
            500,
        )
...
def count_all_documents():
    """Counts the total number of documents across all collections"""
    print("==========================================")
    print("Recevied request for: /count-all-documents")
    print("==========================================")

    if persistentChromaClient is None:
        print(
            "   FROM: /count-all-documents   ChromaDB server is unavailable - Error 500"
        )
        return jsonify({"error": "ChromaDB server is unavailable"}), 500

    try:
        # Check ChromaDB version
        print("   FROM: /count-all-documents   Checking ChromaDB Version...")
        chromadb_version = get_chromadb_app_version()
        if chromadb_version is None:
            print(
                "   FROM: /count-all-documents   ERROR: Could not determine ChromaDB version"
            )
            return jsonify({"error": "Could not determine ChromaDB version"}), 500

        print(
            f"   FROM: /count-all-documents   Confirmed! - ChromaDB version: {chromadb_version}"
        )
        ...
        print("   FROM: /count-all-documents   Fetching all Collections...")
        all_collections = persistentChromaClient.list_collections()
        print("   FROM: /count-all-documents   Collections Fetched")
        ...
        if version_tuple < threshold_version:
            print("   FROM: /count-all-documents   Using logic for ChromaDB < 0.6.0")
            ...
            for collection in all_collections:
                try:
                    ...
                    if collection.count() is not None:
                        ...
                        print(
                            f"   FROM: /count-all-documents   Collection {collection_name} is invalid or empty."
                        )
                except Exception as e:
                    print(
                        f"   FROM: /count-all-documents   Error accessing collection {collection.name}: {str(e)}"
                    )
        else:
            print("   FROM: /count-all-documents   Using logic for ChromaDB 0.6.0+")
            # Loop through all collections to sum their document counts (for 0.6.0+)
            for collection in all_collections:
                try:
                    # Attempt to get the collection
                    temp_collection = persistentChromaClient.get_collection(
                        collection, embedding_function=pyEmbedFunction
                    )

                    # Avoid trying to count documents in empty or corrupted collections
                    if temp_collection.count() is not None:
                        document_count = temp_collection.count()
                        total_document_count += document_count
                    else:
                        # Handle the case where collection count is invalid or empty
                        print(
                            f"   FROM: /count-all-documents   Collection {collection} is invalid or empty."
                        )
                except Exception as e:
                    print(
                        f"   FROM: /count-all-documents   Error accessing collection {collection}: {str(e)}"
                    )

        # Return the total document count as a JSON response
        print(
            f"   FROM: /count-all-documents   Total Document Count: {total_document_count}"
        )
        print("   FROM: /count-all-documents   Returned Count as JSON")
        return jsonify({"total_document_count": total_document_count})

    except Exception as e:
        # Handle any error that occurs during the ChromaDB request
        print(
            f"   FROM: /count-all-documents   Error counting Total Documents from all Collections: {str(e)}"
        )
        return jsonify({"error": str(e)}), 500
...
def get_all_documents():
    ...
    for i, doc_id in enumerate(results["ids"]):
        ...
        embedding = (
            ...
            if isinstance(results["embeddings"][i], np.ndarray)
...
def delete_document():
    ...
    try:
        ...
        return (
            jsonify(
                {
                    "message": f"Document with ID {doc_id_to_delete} has been deleted successfully."
                }
            ),
            200,
        )
...
def delete_all_documents():
    ...
    print("Deleting Collection to Free Up Collection Name...")
    try:
    ...
    try:
        if old_collection_metadata:  # If metadata is not empty
            persistentChromaClient.create_collection(
                passed_collection_name,
                embedding_function=pyEmbedFunction,
                metadata=old_collection_metadata,
            )
        ...
        return (
            jsonify(
                {
                    "message": f"Documents deleted and collection recreated for '{passed_collection_name}' successfully."
                }
            ),
            200,
        )
        ...
        return (
            jsonify(
                {
                    "error": f"Failed to Recreate collection when deleting all documents: {str(e)}"
                }
            ),
            500,
        )
...
def query_documents():
    """Queries documents from a specified collection based on input text and optional filters"""
    # Get the data from the request body
    ...
    if passed_where_document_filters is None:
        print("No Document filters were passed, continuing without....")
    else:
    ...
    if passed_where_metadata_filters is None:
        print("No Metadata filters were passed, continuing without....")
    else:
    ...
    if not passed_collection_name or not query_text:
        return (
            jsonify(
                {
                    "error": "Missing required fields (passed_collection_name, query_text)"
                }
            ),
            400,
        )
...
def update_document():
    ...
    try:
        ...
        return (
            jsonify(
                {
                    "message": f"Document with ID {passed_doc_id} has been updated successfully."
                }
            ),
            200,
        )
...
def visualize_my_collection():
    """Visualizes a specified collection using ChromaViz"""
    ...
    print(
        f"Global variable at start of Visualisation 2 Chroma Viz Port:{CHROMAVIZ_PORT}"
    )
    ...
    print(
        f"Endpoint in ChromaViz Javascript File:\n'{chromaviz_js_file_path}'\nHas been updated."
    )
    print(
        "-------------------------------------------------------------------------------------"
    )
    ...
    print(
        f"Endpoint in ChromaViz index.html:\n'{chromaviz_index_html_path}'\nHas been updated."
    )
    print(
        "-------------------------------------------------------------------------------------"
    )
...
def clear_console():
    """Clears the Flask server console/terminal"""
